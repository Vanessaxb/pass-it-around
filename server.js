...
app.get("/:number_of_bottles", (req, res) => {
  ...
  if (numberOfBottles >= 2) {
    res.send(
      `${numberOfBottles} bottles on the wall.` +
        `<a href="/${nextNumberOfBottles}"> Take one down!</a>`
    );
    return;
  }
  if (numberOfBottles === 1) {
    res.send(
      `${numberOfBottles} bottle on the wall` +
        `<a href="/${nextNumberOfBottles}"> Take one down!</a>`
    );
    return;
  }
  if (numberOfBottles === 0) {
    res.send(
      `${numberOfBottles} bottles on the wall.` + `<a href="/"> Start over!</a>`
    );
    return;
  }
